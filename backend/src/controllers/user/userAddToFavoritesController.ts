...
export const userAddToFavoritesController = [
  ...
  expressAsyncHandler(async function (req: Request, res: Response) {
    ...
    if (type === 'experience') {
      ...
      if (experienceExists) {
        const err = new Error('Experience already exists in favorites.');
        logger.error(err.message);
        res.status(404).json({ message: err.message });
        return;
      }
      ...
      if (groupExists) {
        const err = new Error('Group already exists in favorites.');
        logger.error(err.message);
        res.status(404).json({ message: err.message });
        return;
      }
      ...
      if (memberExists) {
        const err = new Error('Member already exists in favorites.');
        logger.error(err.message);
        res.status(404).json({ message: err.message });
        return;
      }
